    ...
    public class SPTools
    {
            ...
            SPSpriteImportData[] spritesImportData)
        {
            ...
            for (int i = 0; i < uvs.Length; i++)
            {
                ...
                {
                    SpriteMetaData currentMeta =
                        SPTools.GetSpritesheetMeta(texImporter.spritesheet, spritesImportData[i].name);
            ...
            AssetDatabase.SaveAssets();
            SPTools.DoAssetReimport(assetPath,
                ImportAssetOptions.ForceUpdate | ImportAssetOptions.ForceSynchronousImport);
        ...
        public static int ReplaceReferencesInScene(List<SPSpriteInfo> spriteInfoList,
            SPReferenceReplacerWindow.ReplaceMode replaceMode, bool spriteRenderersOnly)
        {
            ...
            foreach (SPSpriteInfo spriteInfo in spriteInfoList)
            {
                ...
                count += SPTools.ReplaceReferences(comps,
                    (replaceAtlas ? spriteInfo.targetSprite : (spriteInfo.source as Sprite)),
                    (replaceAtlas ? (spriteInfo.source as Sprite) : spriteInfo.targetSprite), spriteRenderersOnly);
            }
        ...
        /// <param name="spriteRenderersOnly">If set to <c>true</c> sprite renderers only.</param>
        /// <returns>The replaced references count.</returns>
        public static int ReplaceReferencesInProject(List<SPSpriteInfo> spriteInfoList,
            SPReferenceReplacerWindow.ReplaceMode replaceMode, bool spriteRenderersOnly)
        {
        ...
        /// <returns>The replaced references count.</returns>
        public static int ReplaceReferencesInAllScenes(List<SPSpriteInfo> spriteInfoList,
            SPReferenceReplacerWindow.ReplaceMode replaceMode, bool spriteRenderersOnly, bool skipCurrent)
        {
            int count = 0;
            bool replaceAtlas = (replaceMode == SPReferenceReplacerWindow.ReplaceMode.AtlasWithSource);
            ...
            foreach (string sceneName in sceneNames)
            {
                ...
                if (EditorApplication.OpenScene(sceneName))
                {
                    ...
                    foreach (SPSpriteInfo spriteInfo in spriteInfoList)
                    {
                        if (spriteInfo.source == null || !(spriteInfo.source is Sprite) ||
                            spriteInfo.targetSprite == null)
                            continue;

                        count += SPTools.ReplaceReferences(comps,
                            (replaceAtlas ? spriteInfo.targetSprite : (spriteInfo.source as Sprite)),
                            (replaceAtlas ? (spriteInfo.source as Sprite) : spriteInfo.targetSprite),
                            spriteRenderersOnly);
                    }
        ...
        /// <param name="spriteRenderersOnly">If set to <c>true</c> sprite renderers only.</param>
        /// <returns>The replaced references count.</returns>
        public static int ReplaceReferences(Component[] components, Sprite find, Sprite replaceWith,
